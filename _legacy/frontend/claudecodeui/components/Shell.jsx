...
function Shell({ selectedProject, selectedSession, isActive }) {
  ...
  useEffect(() => {
    ...
    // Add keyboard shortcuts for copy/paste (using Shift+Ctrl to avoid conflicts with shell)
    terminal.current.attachCustomKeyEventHandler((event) => {
      // Shift+Ctrl+C for copy (when text is selected)
      if (event.shiftKey && (event.ctrlKey || event.metaKey) && event.key === 'C' && terminal.current.hasSelection()) {
      ...
      if (event.shiftKey && (event.ctrlKey || event.metaKey) && event.key === 'V') {
